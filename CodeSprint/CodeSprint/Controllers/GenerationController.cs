            ...
            try
            {
                ...
                foreach (var file in files)
                {
                    try
                    {
                        ...
                        string aiResponse = await _codeGenerator.GenerateFromOneFileAsync(tempFilePath, additionalInformation ?? "");

                        // Parse the JSON response from AI to extract questions
                        if (!string.IsNullOrWhiteSpace(aiResponse))
                        {
                            // Try to parse the JSON response
                            try
                            {
                                var questions = System.Text.Json.JsonSerializer.Deserialize<List<object>>(aiResponse);
                                if (questions != null)
                                {
                            ...
                            {
                                // If JSON parsing fails, treat the response as raw data
                                _logger.LogWarning("Failed to parse JSON response for file: {FileName}", file.FileName);
                                
                                // Add as a single object with the raw response
                ...
                if (allQuestions.Count == 0 && failedFiles.Count > 0)
                {
                    return BadRequest(new
                    {
                        success = false,
                ...
                {
                    success = true,
                    message = $"Successfully processed {processedFiles.Count} file(s)",
                    totalFiles = files.Count,
                    successfulFiles = processedFiles.Count,
                    failedFiles = failedFiles.Count,
                    data = allQuestions, // This contains the JSON array that frontend expects
                    processedFileNames = processedFiles,
                    errors = failedFiles.Count > 0 ? failedFiles : null,
                    processedAt = DateTime.UtcNow
                });
            }
            catch (Exception ex)
